...
function pseudoMoves(board,from,attacksOnly=false,player1Color=0){
...
function isSquareAttacked(board,square,byColor,player1Color=0){for(let i=0;i<64;i++)if(board[i]?.color===byColor&&pseudoMoves(board,i,true,player1Color).includes(square))return true;return false}
function isInCheck(board,color,player1Color=0){const k=kingIndex(board,color);return k>=0&&isSquareAttacked(board,k,1-color,player1Color)}
function legalMovesFor(board,from,player1Color=0){const p=board[from];if(!p)return[];return pseudoMoves(board,from,false,player1Color).filter(to=>{const z=[...board];if(z[to]?.type==="king")return false;z[to]={...p};z[from]=null;return!isInCheck(z,p.color,player1Color)})}
function allLegalMoves(board,color,player1Color=0){const out=[];for(let i=0;i<64;i++)if(board[i]?.color===color)for(const to of legalMovesFor(board,i,player1Color))out.push([i,to]);return out}
...
function App(){
 ...
 const legal=useMemo(()=>selected!==null&&board[selected]?.color===activeColor?legalMovesFor(board,selected,player1Color):[],[board,selected,activeColor]);
 const inCheck=isInCheck(board,activeColor,player1Color),checkmate=inCheck&&allLegalMoves(board,activeColor,player1Color).length===0,stalemate=!inCheck&&allLegalMoves(board,activeColor).length===0;
 ...
 const move=(to)=>{
   ...
   if(isInCheck(z,opponentColor,player1Color))addLog(`⚠️ Player ${turn===0?2:1} is in CHECK.`);
   if(isInCheck(z,opponentColor)&&allLegalMoves(z,opponentColor,player1Color).length===0){setWinner(turn);setStatus("CHECKMATE");addLog(`♔ CHECKMATE — Player ${turn+1} wins.`);return}
 ...
 useEffect(()=>{
   ...
   const t=setTimeout(()=>{
     const choices=allLegalMoves(board,activeColor,player1Color);if(!choices.length)return;
     ...
     if(isInCheck(z,opponentColor)&&allLegalMoves(z,opponentColor,player1Color).length===0){setWinner(1);setStatus("CHECKMATE");addLog("♔ CHECKMATE — Computer wins.");return}
