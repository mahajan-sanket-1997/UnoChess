 ...
 return <div className="app">
  ...
  <div className="game">
   <section className="board-wrap">
    <div className="playerbar p2"><div className="avatar black">♚</div><div><b>PLAYER 2</b><small>{mode==="computer"?`COMPUTER · ${playerColor(1)===0?"WHITE":"BLACK"}`:`${playerColor(1)===0?"WHITE":"BLACK"}`}</small></div><div className="dots"><i/><i/><i/><i/></div></div>
    <div className="board-frame"><div className="coords top">{["a","b","c","d","e","f","g","h"].map(x=><span>{x}</span>)}</div><div className="board">{squares.map(i=>{const j=flipped?63-i:i,[r,c]=rc(j),p=board[j],can=legal.includes(j),isSel=selected===j,last=lastMove?.includes(j);return <button key={i} className={`sq ${(r+c)%2?"dark":"light"} ${can?"legal":""} ${isSel?"selected":""} ${last?"last":""}`} onClick={()=>{p?.color===activeColor&&!(mode==="computer"&&turn===1){setSelected(j);return}if(moves&&selected!==null&&can)move(j)}}>{p&&<span className={`chess-piece ${p.color===0?"white-piece":"black-piece"}`}>{PIECES[p.type].symbol}</span>}{can&&<span className="move-dot"/>}</button>})}</div><div className="coords bottom">{["a","b","c","d","e","f","g","h"].map(x=><span>{x}</span>)}</div></div>
    <div className="playerbar p1"><div className="avatar white">♔</div><div><b>PLAYER 1</b><small>{player1Color===0?"WHITE":"BLACK"} · YOU</small></div><div className="dots"><i/><i/><i/><i/></div></div>
   </section>
   <aside>
