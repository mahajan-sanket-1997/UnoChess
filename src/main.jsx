...
function App(){
 ...
 const inCheck=isInCheck(board,turn),checkmate=inCheck&&allLegalMoves(board,turn).length===0,stalemate=!inCheck&&allLegalMoves(board,turn).length===0;
 const addLog=m=>setLog(x=>[m,...x].slice(0,7));
 const reset=()=>{setBoard(initialBoard());setTurn(0);setDeck(makeDeck());setDiscard([]);setCard(null);setMoves(0);setSelected(null);setWinner(null);setStatus("");setFlipped(false);setLastMove(null);setLog(["New game started. Draw a card."]);setMode(null)};
 ...
 const draw=()=>{
   ...
   else if(c.type==="reverse"){setFlipped(x=>!x);addLog(`Player ${turn+1} played REVERSE — board flipped.`);setTimeout(nextTurn,600)}
 ...
 const move=(to)=>{
   ...
   const nextMoves=moves-1;if(isInCheck(z,1-turn))addLog(`⚠️ Player ${2-turn} is in CHECK.`);
 ...
 useEffect(()=>{
   ...
   const t=setTimeout(()=>{
     ...
     if(isInCheck(z,0))addLog("⚠️ Player 1 is in CHECK.");
     if(isInCheck(z,0)&&allLegalMoves(z,0).length===0){setWinner(1);setStatus("CHECKMATE");addLog("♔ CHECKMATE — Computer wins.");return}
     if(moves-1===0)setTimeout(nextTurn,400);
   },650);return()=>clearTimeout(t)
 },[mode,turn,winner,moves,board,deck,discard]);
 ...
 return <div className="app">
  <header><div className="brand"><div><span>UNO</span><b>CHESS</b></div><small>CARD-POWERED CHESS BATTLE</small></div><div className="top-actions"><button onClick={()=>setFlipped(x=>!x)}>↻ FLIP BOARD</button><button onClick={()=>setMode(null)}>NEW GAME</button></div></header>
  <div className="game">
   <section className="board-wrap">
    <div className="playerbar p2"><div className="avatar black">♚</div><div><b>PLAYER 2</b><small>{mode==="computer"?"COMPUTER · BLACK":"BLACK"}</small></div><div className="dots"><i/><i/><i/><i/></div></div>
    <div className="board-frame"><div className="coords top">{["a","b","c","d","e","f","g","h"].map(x=><span>{x}</span>)}</div><div className="board">{squares.map(i=>{const j=flipped?63-i:i,[r,c]=rc(j),p=board[j],can=legal.includes(j),isSel=selected===j,last=lastMove?.includes(j);return <button key={i} className={`sq ${(r+c)%2?"dark":"light"} ${can?"legal":""} ${isSel?"selected":""} ${last?"last":""}`} onClick={()=>{if(p?.color===turn&&!(mode==="computer"&&turn===1)){setSelected(j);return}if(moves&&selected!==null&&can)move(j)}}>{p&&<span className={`chess-piece ${p.color===0?"white-piece":"black-piece"}`}>{PIECES[p.type].symbol}</span>}{can&&<span className="move-dot"/>}</button>})}</div><div className="coords bottom">{["a","b","c","d","e","f","g","h"].map(x=><span>{x}</span>)}</div></div>
    <div className="playerbar p1"><div className="avatar white">♔</div><div><b>PLAYER 1</b><small>WHITE · YOU</small></div><div className="dots"><i/><i/><i/><i/></div></div>
   </section>
   <aside>
    <div className="turn-panel"><small>CURRENT TURN</small><div className="turn"><div className={`turn-avatar ${turn?"black":"white"}`}>{turn?"♚":"♔"}</div><div><b>{mode==="computer"&&turn===1?"COMPUTER":`PLAYER ${turn+1}`}</b><span>{turn?"BLACK":"WHITE"}</span></div></div><hr/><small>MOVES LEFT</small><strong className="move-count">{moves}</strong>{status&&<div className={`status ${status.toLowerCase()}`}>{status}</div>}</div>
    ...
    <div className="rules"><b>HOW TO PLAY</b><p>Number cards are only <strong>1, 2 or 3</strong>.</p><p>SKIP skips the opponent's turn. REVERSE flips the board.</p><p>+2 and +4 give 2 or 4 chess moves.</p><p>Chess rules apply: you cannot leave your own king in check.</p><p>King is never captured. Checkmate wins; stalemate is a draw.</p></div>
